...
{
    public class CurveRouteTests : IClassFixture<CustomWebApplicationFactory>
    {
        private readonly HttpClient _client;

        public CurveRouteTests(CustomWebApplicationFactory factory)
        {
        ...
        {
            var response = await _client.GetAsync("/Curve/list");
        ...
        {
            var response = await _client.GetAsync("/Curve/1");
            Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
        }
        ...
        {
            SetToken("User");
            var response = await _client.GetAsync("/Curve/999");
        ...
        {
            SetToken("Admin");
            var response = await _client.PutAsync("/Curve/update/999",
                JsonBody(new CurveDTO { CurveId = 1, Term = 1.0 }));
            Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
        }
        ...
        {
            SetToken("Admin");
            var response = await _client.DeleteAsync("/Curve/999");
            Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
        }
        ...
        {
            SetToken("Admin");
            await _client.PostAsync("/Curve/validate",
                JsonBody(new CurveDTO { CurveId = 1, Term = 1.0 }));
            var response = await _client.DeleteAsync("/Curve/1");
