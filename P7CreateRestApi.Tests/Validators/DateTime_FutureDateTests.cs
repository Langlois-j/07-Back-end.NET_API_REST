...
{
    public class DateTime_FutureDateTests
    {
        private ValidationResult? Validate(DateTime? value, bool allowToday = true)
        {
            var attr = new DateTime_FutureAttribute(null,allowToday);
            return attr.GetValidationResult(value, new ValidationContext(new { }));
        }

        [Fact]
        public void ShouldPass_WhenValueIsInFuture()
        {
        ...
        public void ShouldFail_WhenValueIsToday_AndAllowTodayIsFalse()
        {
            ...
            Assert.Contains(DateTime_FutureAttribute.ErrorCodePastDate, result!.MemberNames);
        }

        [Fact]
        public void ShouldFail_WhenValueIsInPast()
        {
            var result = Validate(DateTime.UtcNow.AddDays(-1));
            Assert.NotNull(result);
            Assert.Contains(DateTime_FutureAttribute.ErrorCodePastDate, result!.MemberNames);
